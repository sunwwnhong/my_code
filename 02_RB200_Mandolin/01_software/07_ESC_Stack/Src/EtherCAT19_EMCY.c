...
#define    _EMCY_    1
#include "EtherCAT19_EMCY.h"
#undef      _EMCY_
/*remove definition of _EMCY_ (#ifdef is used in emcy.h)*/
#include "EtherCAT08_EthCOE.h"

#include "EtherCAT07_EMCYCode.h"
...
/****************************************************************************************************/
/* Function name:ServoEmcyCheck                                                                     */
/* Function Description:紧急报文核对和上传                                                          */
/****************************************************************************************************/
USHORT EmcyDelayCnt1 = 0;
USHORT EmcyDelayCnt2 = 0;
USHORT MotB_EmcyFlag = 0;
void ServoEmcyCheck( void )
{
    TEMCYMESSAGE  Emcy_message;
    TEMCYMESSAGE  *pEmcy_message;
    pEmcy_message = &Emcy_message;

    if( EtherCATErrorFlag == 1 )
    {
        if( ++EmcyDelayCnt1 >= 10 )
        {
            if( ( nAlStatus & STATE_MASK ) >= 0x02 )
            {
                EmcyDelayCnt1 = 0;
                EtherCATErrorFlag = 0;
                Emcy_message.Emcy.Code = EtherCATErrorCode;               // 错误码 OBJ:0x603F  u16ErrCode;
                Emcy_message.Emcy.RegData[0] = 0;                         // 低8位错误寄存器 OBJ:0x1001,暂时默认为0;高8位报警辅助编号,暂时默认为0
                Emcy_message.Emcy.RegData[1] = EtherCATErrorAddCode;      // 厂商指定错误码
                Emcy_message.Emcy.RegData[2] = 0;                         // 厂商指定错误码
                EMCY_SendEmergency( pEmcy_message );                      // 发送
            }
            else
            {
                EmcyDelayCnt1 = 0;
