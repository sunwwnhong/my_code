...
#include "EtherCAT10_EthAppl.h"


#include "EtherCAT05_mailbox.h"
...
//    HW_EscRead((MEM_ADDR *)&SyncManInfo, ESC_SYNCMAN_REG_OFFSET + (channel * SIZEOF_SM_REGISTER), SIZEOF_SM_REGISTER );
    ...
    UINT16 Address = ESC_SYNCMAN_REG_OFFSET + (channel * SIZEOF_SM_REGISTER);

    HW_EscReadWord(SyncManInfo.PhysicalStartAddress,Address);

    HW_EscReadWord(SyncManInfo.Length,Address + 2 );

    HW_EscReadWord(SyncManInfo.Settings[0],Address + 4);

    HW_EscReadWord(SyncManInfo.Settings[1],Address + 6 );
