...
UINT8 OBJ_Read( UINT16 index, UINT8 subindex, UINT32 objSize, OBJCONST TOBJECT OBJMEM * pObjEntry, UINT16 MBXMEM * pData, UINT8 bCompleteAccess )
{
    ...
    UINT16 Mot2ObjFlag = 0;
    UINT32 AddressOffSet = (UINT32)(pObjEntry->pVarPtr);
    if( AddressOffSet >= 0x0003A000 && AddressOffSet <= 0x0003A064 )
    {
        Mot2ObjFlag = 1;
    }
    ...
    if ( objCode != OBJCODE_VAR && index >= 0x1000 )
    {
        ...
        object's variable */
#if 0
        maxSubindex = Mot2ObjFlag ? ((*((UINT16 *) (AddressOffSet + 0x00001000))) & 0x00FF) :\
                                     (*((UINT16 *) (AddressOffSet))) & 0x00FF;

#else
        if( ((index & 0x1000) == 0x1000) || ((index & 0x6000) == 0x6000 ))
        {
            maxSubindex = Mot2ObjFlag ? ((*((UINT16 *) (AddressOffSet + 0x00001000))) & 0x00FF) :\
                                         (*((UINT16 *) (AddressOffSet))) & 0x00FF;
        }
