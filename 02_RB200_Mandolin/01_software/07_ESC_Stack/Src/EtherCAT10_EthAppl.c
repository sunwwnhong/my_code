...
BOOL bInitFinished = 0; /** < \brief indicates if the initialization is finished*/
...
#ifdef RunRAM
#pragma CODE_SECTION(Sync0_Isr, ".TI.ramfunc");
#endif
void Sync0_Isr(void)
{
     Sync0WdCounter = 0;
    ...
    if(bDcSyncActive)
    {
        ...
        {
            if ( bEcatInputUpdateRunning ) /* Inputs shall be latched on a specific Sync0 event */
            {
                /* EtherCAT slave is at least in SAFE-OPERATIONAL, update inputs */
                //DISABLE_ESC_INT();
                PDO_InputMapping();
